...
typename GridType::Ptr exportVDB(Grid<T>* from, float clip, openvdb::FloatGrid::Ptr clipGrid) {
	...
	if (true) {
		...
		openvdb::math::CoordBBox bbox(openvdb::Coord(0), openvdb::Coord(from->getSizeX() - 1, from->getSizeY() - 1, from->getSizeZ() - 1));
		openvdb::tools::Dense<ValueT, openvdb::tools::MemoryLayout::LayoutXYZ> dense(bbox, data);

		// Trick: Set clip value to very small / negative value in order to copy all values of dense grids
...
int writeObjectsVDB(const string& filename, std::vector<PbClass*>* objects, float worldSize, bool skipDeletedParts, int compression, int precision, float clip, const Grid<Real>* clipGrid) {
	openvdb::initialize();
	...
	if (clipGrid) {
		...
		openvdb::math::CoordBBox bbox(openvdb::Coord(0), openvdb::Coord(clipGrid->getSizeX() - 1, clipGrid->getSizeY() - 1, clipGrid->getSizeZ() - 1));
	...
	for (std::vector<PbClass*>::iterator iter = objects->begin(); iter != objects->end(); ++iter) {
		...
		if (GridBase* mantaGrid = dynamic_cast<GridBase*>(*iter)) {
			...
			if (clipGrid) {
				assertMsg(clipGrid->getSize() == mantaGrid->getSize(), "writeObjectsVDB: Clip grid and exported grid must have the same size");
			}
...
template openvdb::Int32Grid::Ptr exportVDB<int, openvdb::Int32Grid>(Grid<int> *from, float clip=1e-4, openvdb::FloatGrid::Ptr clipGrid=nullptr);
template openvdb::FloatGrid::Ptr exportVDB<Real, openvdb::FloatGrid>(Grid<Real> *from, float clip=1e-4, openvdb::FloatGrid::Ptr clipGrid=nullptr);
template openvdb::Vec3SGrid::Ptr exportVDB<Vec3, openvdb::Vec3SGrid>(Grid<Vec3> *from, float clip=1e-4, openvdb::FloatGrid::Ptr clipGrid=nullptr);
