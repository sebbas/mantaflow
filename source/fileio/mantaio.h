...
// OpenVDB
int writeObjectsVDB(const std::string& filename, std::vector<PbClass*>* objects, float scale=1.0, bool skipDeletedParts=false, int compression=COMPRESSION_ZIP, int precision=PRECISION_HALF, float clip=1e-4, const Grid<Real>* clipGrid=nullptr);
...
template<class T> int readGrid4dUni (const std::string& name, Grid4d<T>* grid, int readTslice=-1, Grid4d<T>* slice=nullptr, void** fileHandle=nullptr);
...
// Helpers
void getUniFileSize(const std::string& name, int& x, int& y, int& z, int* t = nullptr, std::string* info = nullptr);
